...
with ``LGBMRegressor``.  Feature engineering is delegated to ``FeatureEngineer``;
forecasting uses the same recursive one-step-at-a-time strategy as
``XGBoostSpec`` and ``RandomForestSpec``.
...
class LightGBMSpec:
    ...
    def evaluate(
        ...
        3. Trains ``LGBMRegressor`` with early stopping on the inner val set.
        ...
        for k in range(_N_CV_FOLDS):
            ...
            try:
                ...
                logger.debug("LightGBMSpec.evaluate fold %d failed: %s", k + 1, exc)
    ...
    def build_forecaster(
        ...
        The returned callable fits an ``LGBMRegressor`` on *train* using the
